...
#위의 코드가 from sklearn.impute import Imputer에서 from sklearn.impute import SimpleImputer로 바껴서 맞춰주었다
#SimpleImputer(missing_values= np.NAN, strategy= 'mean', fill_value=None, verbose=0, copy=True)
...
def make_X(molecular_data,kinds,target_dilution=None,threshold=None,
    ...
    CID_dilutions = []
    for kind in kinds:
        #밑의 코드는 kind가 in ['training','leaderboard','testset']안에 좋재하면 다음 문장이 실행되고 아니면 멈춤
        #\이게 무슨 뜻이지?
        assert kind in ['training','leaderboard','testset'], \
            "No such kind %s" % kind
