...
def load_perceptual_data(kind):
    ...
    with open('%s.txt' % kind) as f:
        ...
        for line_num,line in enumerate(reader):
            #아래의 코드를 이용해서 데이터를 나눠준다. headers는 첫번째 행으로 이름들을 나타낸다. 밑에 데이터들은 data로 봔환되는데 말그래도 data다. 
            if line_num > 0:
                #x.strip은 공백을 없애준다. 왜하는지는 모름 
...
def load_leaderboard_perceptual_data(target_dilution=None):
    ...
    #아래의 _는 headers만 받고 data는 받지 않기 위해서 저렇게 표현한 것이다. 
    perceptual_headers, _ = load_perceptual_data()
    #아래의 코드는 (percepcual_deaders는 위에서 말한 것처럼 데이터의 0번째행즉 이름들만 받아왔다.)이중에 6번째부터 쓰겠다는 말이다. 
...
def get_CID_dilutions(kind,target_dilution=None):
    ...
    if kind == 'training':
        ...
        #아래의 코드는 header는 안받고 data만 받는다는말
        #아래의 코드를 실행해 보니 [[],[],[]...]이런 식으로 파일이 만들어지게된다. [[],[],[],,,] 전체는 lines이고 안의 []하나하나는 line이다. 
        _,lines = load_perceptual_data(kind)
        for line in lines[1:]:
            CID = int(line[0])
            dilution = line[4]
            #위에서 얻어진 dilution값이 아래에 들어가게된다. 
